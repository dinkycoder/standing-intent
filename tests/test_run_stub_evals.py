...
def test_writes_one_report_per_task(tmp_path):
    ...
    for path in written:
        ...
        if report.task_id in (
            "no_in_policy_vendor_escalates",
            "price_anomaly_escalates",
            "vendor_down_no_fallback_escalates",
        ):
            # The stub always escalates with zero purchases -- which happens
            # to be the correct terminal state for all three of these tasks
            # (no valid purchase exists), so it passes here for the right
            # STATE but the wrong REASON: it never distinguishes a real
            # vendor set from an empty one, a genuine price anomaly from a
            # normal offer, or a permanently-down vendor from a reachable
            # one it simply never tried. See the Week-5 design spec's
            # "Known limitations".
